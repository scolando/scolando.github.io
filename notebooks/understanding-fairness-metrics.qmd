...
format:
  html:
    ...
    toc-title: 'Contents'
bibliography: alg-fairness/alg_fairness.bib
---

::: {style="float: right; margin: 1em;"}
![A slightly cynical perspective on outcome-based<br>algorithmic fairness metrics from [Machines Gone Wrong](https://machinesgonewrong.com/fairness/).](alg-fairness/manymetrics.png){#fig-manymetrics width="350"}
:::

Artificial intelligence (AI) is becoming an increasingly ubiquitous component of decision-making. From an ethics perspective, this raises concerns since many of decisions that involve AI have high moral stakes. For instance, predictive AI is commonly used to evaluate loan and mortgage requests, [[cite]], approve health insurance claims [[cite]], and help judges assess the pre-trial risk for convicted individuals [[cite]]. To this end, it is important to understand whether using a given AI tool in such settings is *unfair*. 

Since predictive AI (which is the type of AI that I focus on in this post) uses patterns in the training data to build a statistical model that can then be applied to cases outside the original training set (p. 3) [[cite]], a natural approach for assessing algorithmic fairness is to use already-developed model evaluation metrics that seem to capture some aspect of "fairness". These metrics are often *outcome-based* in the sense that they evaluate whether a predictive AI is *unfair* based on its predictions rather based on its underlying statistical model. Several outcome-based measures of algorithmic fairness has been proposed (see @fig-manymetrics) since the fair machine learning field exploded in the mid-2010s. Three of the most popular outcome-based measures of algorithmic fairness for AI are: 
...
However, [[cite]] show that metrics (1)-(3) cannot be mutually satisfied except in pathological cases (i.e., ___ or ____). [[cite]] coin this as an *impossibility result* since they 
...
![Confusion Matrix Diagram from Analytics Vidhya.](alg-fairness/confusion-matrix.png){#fig-confusion width="350"}
:::
